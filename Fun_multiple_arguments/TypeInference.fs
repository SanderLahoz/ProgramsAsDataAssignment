...
let debug s =
    if debugP then printfn "DEBUG: %s" s else ()
...
type typ =
    | TypI (* integers                   *)
    | TypB (* booleans                   *)
    | TypF of typ list * typ (* (argumenttypes, resulttype)  *)
...
let rec freeTypeVars t : typevar list =
    ...
    | TypB -> []
    | TypV tv -> [ tv ]
    | TypF(ts, t2) -> List.foldBack (fun t acc -> union (freeTypeVars t, acc)) ts (freeTypeVars t2)
...
let rec showType t : string =
    let rec pr t =
        ...
        | TypF(ts, t2) -> "(" + String.concat " * " (List.map pr ts) + " -> " + pr t2 + ")"
...
let rec showTEnv tenv =
    ...
    let showTypescheme =
        ...
        | TypeScheme(tvs, typ) ->
            "V"
            + (List.foldBack (fun tv a -> (showTyvar tv) + "." + a) tvs "")
            + (showType typ)
...
let rec unify t1 t2 : unit =
    ...
    let _ =
        debug ("Unify called with t1=" + (showType t1) + " and t2= " + (showType t2))

    match (t1', t2') with
    | (TypI, TypI) -> ()
    | (TypB, TypB) -> ()
    | (TypF(ts1, t12), TypF(ts2, t22)) ->
        if List.length ts1 <> List.length ts2 then
            failwith "type error: functions with different number of arguments"
...
let rec copyType subst t : typ =
    ...
    | TypV tyvar ->
        ...
        loop subst
    | TypF(ts, t2) -> TypF(List.map (copyType subst) ts, copyType subst t2)
    | TypI -> TypI
    | TypB -> TypB
...
let rec typ (lvl: int) (env: tenv) (e: expr) : typ =
    ...
    | Letfun(f, xs, fBody, letBody) ->
        ...
        let xTyps = List.map (fun _ -> TypV(newTypeVar lvl1)) xs

        let fBodyEnv =
            List.map2 (fun x xTyp -> (x, TypeScheme([], xTyp))) xs xTyps
            @ (f, TypeScheme([], fTyp)) :: env

        let rTyp = typ lvl1 fBodyEnv fBody
        let _ = unify fTyp (TypF(xTyps, rTyp))
    ...
    | Call(eFun, eArgs) ->
        ...
        let txs = List.map (typ lvl env) eArgs
